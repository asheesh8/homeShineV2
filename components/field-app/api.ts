...
/** Parse JSON safely — returns null on parse errors (e.g. HTML error pages) */
...
export async function fetchAssessments(): Promise<Assessment[]> {
  ...
  const data = await safeJson(res) as Assessment[] | { error: string };
...
export async function requestAiSummary(
  ...
  const data = await safeJson(res) as { summary: string; nextSteps: string[]; sources: import("@/lib/simple-field").AiSource[] } | { error: string };
